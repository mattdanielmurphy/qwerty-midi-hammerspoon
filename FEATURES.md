...
- **Logic Pro Session BPM Sync**: Full bi-directional real-time synchronization between active Logic Pro session BPM and the Arpeggiator engine via AppleScript/JSA Accessibility interface (`AXIncrement`/`AXDecrement`), supporting both auto-fetching Logic tempo and pushing controller BPM changes directly into Logic Pro.
