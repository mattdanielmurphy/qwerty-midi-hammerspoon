...
- **Dedicated Number Row Arp & Synth Controls**: Number keys `1` through `=` mapped to comprehensive Arp, Synth Release, & BPM controls (`1`: Arp On/Off, `2`: Top Arp, `3`: Bot Arp, `4`/`5`: Dir -/+, `6`/`7`: Rate -/+, `8`/`9` Gate -/+, `9`/`0`: Synth Release -/+, `-`/`=`: BPM -/+). Holding **Shift** accesses alternate controls (Panic, Transpose, Octave, Mode, Zoom, Reset).
...
- **Correlated Control Key Pair Styling**: Related control keys (e.g. `J`/`K` Transpose, `H`/`L` Root, `G`/`;` Mode, `D`/`F` Octave, `3`/`4` Dir, `5`/`6` Rate, `7`/`8` Gate, `9`/`0` Release, `-`/`=` BPM) share cohesive, understated color accents matching the warm dark HUD theme for effortless visual grouping without dominating note keys.
