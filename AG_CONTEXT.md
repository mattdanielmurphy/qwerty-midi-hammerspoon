...
- `packages/nanokey-studio/`: Korg nanoKEY Studio hardware driver with CC #25 Sustain & native SysEx Scene dual hold-to-reveal macro layers, 8 rotary dials mapped to Korg Gadget defaults (Cutoff, Peak, Drive, Volume, ADSR) with adaptive offset resolution (CC 17..24, 16..23, 20..27), KAOSS touchpad, 8 velocity trigger pads, 25 chiclet keys with octave-folding real-time visualizer, and dynamic surface switcher.
  ...
  - **Pad MIDI Channel & Notes**: The 8 trigger pads transmit on Channel 2 (`ch = 1`) sending notes 64..71 (E4..B4), or drum channel 10 (`ch = 9`) notes 36..43. In Chord Pad mode, pads emit 3–4 note chords on Channel 1 (`ch = 0`). Single keyboard notes on Channel 1 (`ch = 0`) must never trigger pad highlights.
  - **Physical Keyboard Note Range**: 25 physical keys transmit on Channel 1 (`ch = 0`) spanning notes 36 to 60 (C2 to C4) at factory default, with middle C at note 48 (C3) and high C at note 60 (C4). Octave folding in the GUI maps into the 36..60 window.
