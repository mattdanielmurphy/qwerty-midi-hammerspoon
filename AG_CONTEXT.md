...
- `packages/nanokey-studio/`: Korg nanoKEY Studio hardware driver with CC #64 Sustain & native SysEx Scene dual hold-to-reveal macro layers, 8 rotary dials mapped to Korg Gadget defaults (Cutoff CC 20, Peak CC 21, Drive CC 22, Volume CC 23, Attack CC 24, Decay CC 25, Sustain CC 26, Release CC 27), KAOSS touchpad (X = CC 1, Y = CC 19), 8 velocity trigger pads pre-assigned to CCs (Row 1: 43, 48, 50, 49; Row 2: 36, 38, 42, 46), 25 chiclet keys with octave-folding real-time visualizer, and dynamic surface switcher.
  ...
  - **Inert vs. Interactive Invariant & Native Mode SysEx Protocol**: In factory standard mode, internal firmware functions (`Octave -/+`, `Touch Scale`, `X-Y`, `Pitch / Mod`, `Shift / Tap`, `Arp`, `Chord Pad`, `Easy Scale`, `Scale Guide`) do not emit MIDI over the air. However, transmitting the Korg Native Mode Handshake (`F0 7E 7F 06 01 F7` -> `F0 42 40 00 01 36 01 00 00 12 F7` -> `F0 42 40 00 01 36 02 00 00 00 01 F7`) causes controller LEDs to flash and re-routes MCU function buttons to emit 5-byte BLE-MIDI GATT notification frames on characteristic `7772E5DB-3868-4112-A1A9-F2669D106BF3` (`F0 42 40 00 01 36 05 00 00 41 [subId] [val] 00 00 F7`), allowing host interception of Octave Up (`414001`), Octave Down (`414000`), and Scene (`4140407f`/`41404000`). Factory normal mode is restored via `F0 42 40 00 01 36 02 00 00 00 00 F7`. The standalone calibration CLI `bin/sniff` provides systematic button packet sniffing and verification.
