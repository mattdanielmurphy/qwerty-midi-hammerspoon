...
- `packages/nanokey-studio/`: Korg nanoKEY Studio hardware driver with CC #25 Sustain & native SysEx Scene dual hold-to-reveal macro layers, 8 rotary dials mapped to Korg Gadget defaults (Cutoff, Peak, Drive, Volume, ADSR) with adaptive offset resolution (CC 17..24, 16..23, 20..27), KAOSS touchpad, 8 velocity trigger pads, 25 chiclet keys with octave-folding real-time visualizer, and dynamic surface switcher.
  ...
  - **Pad vs Keyboard Channel Isolation**: Physical keys transmit Note On/Off on Channel 1 (`ch = 0`, notes 48..72). Pads are isolated from keys either via **Control Change on Channel 1 / Global** (CC 80..87 recommended, or CC 102..109, 112..119, 36..43 with Momentary behavior) OR via dedicated Note channels (Channel 10 GM drum standard or Channel 2). CC messages and Note messages use distinct MIDI status bytes, completely eliminating cross-trigger collisions on a single channel. Keyboard notes never trigger pad highlights, and pad hits never trigger keyboard keys.
